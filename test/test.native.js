...
tape( 'the function returns `true` if provided `-0`', opts, function test( t ) {
	t.strictEqual( isNegativeZerof( -0.0 ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `false` if provided `+0`', opts, function test( t ) {
	t.strictEqual( isNegativeZerof( 0.0 ), false, 'returns expected value' );
	t.strictEqual( isNegativeZerof( +0.0 ), false, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns `false` if not provided `-0`', opts, function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( isNegativeZerof( values[i] ), false, 'returns expected value when provided ' + values[ i ] );
	}
	t.end();
});
