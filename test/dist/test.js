...
var isNegativeZerof = require( './../../dist' );
...
tape( 'the function returns `true` if provided `-0`', function test( t ) {
	t.equal( isNegativeZerof( -0.0 ), true, 'returns true' );
	t.end();
});

tape( 'the function returns `false` if provided `+0`', function test( t ) {
	t.equal( isNegativeZerof( 0.0 ), false, 'returns false' );
	t.equal( isNegativeZerof( +0.0 ), false, 'returns false' );
	t.end();
});
...
tape( 'the function returns `false` if not provided `-0`', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.equal( isNegativeZerof( values[i] ), false, 'returns false when provided ' + values[ i ] );
	}
	t.end();
});
